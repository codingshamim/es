  ...
  if (orders?.error) {
    return (
      <div className="p-10 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-gray-400">{orders.message || "Please try again later."}</p>
      </div>
    );
  }
      ...
      <div className="overflow-hidden shadow-2xl">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                {["Order ID", "Customer", "Products", "Amount", "Payment", "Status", "Date", "Actions"].map((head) => (
                  <th
                    key={head}
            ...
            <tbody className="divide-y divide-gray-700" id="orders-table-body">
              {orders.orders.map((order) => {
                const orderTotal = calculateItemTotal(order.orders) + (order.shippingOption?.fee || 0);
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-white font-semibold">
                          {order?.address?.name?.slice(0, 2).toUpperCase() || "NA"}
                        </div>
                        <div className="ml-3">
                          <div className="text-sm font-medium text-white">
