...
export default function ProductItem({
  ...
  return (
    <AnimationContainer>
      <div className="group bg-gray-900 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-800 hover:border-gray-700">
        <div className="relative overflow-hidden">
          <Link href={`/tshirt/${slug}`}>
            <Image
              className="w-full h-[300px] object-cover group-hover:scale-105 transition-transform duration-500 ease-out"
              src={thumbnail}
              alt={title}
              width={1200}
              height={1200}
        ...
        <div className="p-5">
          <Link href={`/tshirt/${slug}`} className="block">
            <h1 className="font-semibold text-white hover:text-blue-400 transition-colors duration-200 line-clamp-2 mb-2">
              {title}
            </h1>
          ...
          <div className="flex items-baseline gap-2 mb-4">
            {discount > 0 && (
              <del className="text-gray-500 text-sm">{mainPrice(price)}</del>
            )}
            ...
            {discount > 0 && (
              <span className="text-green-400 text-sm font-medium">
                Save {mainPrice((price / 100) * discount)}
              </span>
            )}
          </div>
