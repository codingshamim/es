...
export default function CheckoutSubmitter({ children, cartItems = [],totalPrice }) {
  ...
  const handleCheckout = async (event) => {
    ...
    try {
      ...
      if (validCartItems.length !== cartItems.length) {
        setSubmitError("Some items in your cart are invalid. Please refresh and try again.");
        return;
      }
      ...
      if (checkoutResponse?.error) {
        setSubmitError(checkoutResponse.message || "Failed to process checkout. Please try again.");
      } else {
        router.push(`/order-success?transactionId=${checkoutResponse?.data?.transactionId}&totalAmount=${totalPrice}&name=${checkoutResponse?.data?.name}`);
        setSubmitSuccess("Order placed successfully! You will receive a confirmation shortly.");
  ...
  const inputBaseClass = "bg-black px-4 py-2 w-full text-white text-sm border transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500";

  const getInputClass = (field) =>
    `${inputBaseClass} ${errors[field] ? "border-red-500 focus:ring-red-500" : "border-gray-600 focus:border-blue-500"}`;
      ...
      <div className="border border-gray-700 p-4 space-y-4 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {errors.district && (
              <p id="district-error" className="text-red-500 text-xs mt-1" role="alert">
                {errors.district}
              </p>
            )}
          </div>
